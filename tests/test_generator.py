...
class TestFriend:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                generator.Friend("std::vector<float>", "private"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Foo<float, std::vector<float>>",
            "extern template class Foo<float, std::vector<float>>;",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Foo<float, std::vector<float>>",
            "template class Foo<float, std::vector<float>>;",
        ),
    ],
)
...
@pytest.mark.parametrize(
    "parent, return_type, params, expected",
    [
        ...
        (
            ...
            "float&",
            [
                types.Type(inner="T", const=True),
                types.Type(inner="int", lvalue_ref=True),
            ],
            "::drmock::Method<Bar, float&, T, int>",
        ),
    ],
)
