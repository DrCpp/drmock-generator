...
class TestNode:
    @pytest.mark.skip(
        reason="Can't be tested beyond verifying the method's imperative. Moreover, we're using this method thoroughly in the others tests."
    )
...
class TestTranslate:
    def test_class_template(self, set_library_file):
        root = translator.translate(
            PATH, "template<typename T, typename... Ts> class X {};", ["--std=c++17"]
        )
        ...
        t2 = children[1]
        assert t1.cursor.kind == clang.cindex.CursorKind.TEMPLATE_TYPE_PARAMETER
        assert [each.spelling for each in t2.cursor.get_tokens()] == [
            "typename",
            "...",
            "Ts",
        ]
    ...
    def test_type_alias_template(self, set_library_file):
        root = translator.translate(
            PATH, "template<typename T> using value_type = T;", ["--std=c++11"]
        )
    ...
    @pytest.mark.parametrize(
        "source, spelling, params, tokens, const, virtual, pure_virtual, noexcept",
        [
            (
                "class Dummy { void f(); };",
                "f",
    ...
    def test_cxx_method(
        ...
        assert [
            each.get_tokens()
            for each in cxx_method.get_children()
            if each.cursor.kind == clang.cindex.CursorKind.PARM_DECL
        ] == params
        ...
        assert (
            not noexcept
            or cxx_method.cursor.exception_specification_kind
    ...
    def test_class(self, set_library_file):
        ...
        template_type_params = [
            each.get_tokens()
            for each in node2.get_children()
            if each.cursor.kind == clang.cindex.CursorKind.TEMPLATE_TYPE_PARAMETER
        ]
        ...
        offset = next(
            i
            for i, elem in enumerate(node2.get_children())
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "using value_type = T", ["--std=c++11"], id="unknown type name 'T'"
            ),
        ],
    )
